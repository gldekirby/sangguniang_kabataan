...
// Fetch all projects grouped by sector (category)
$sql = "SELECT p.*, ab.program_name, ab.fiscal_year, fs.name AS fund_name, bc.category_name FROM projects p
        LEFT JOIN annual_budget ab ON p.budget_id = ab.id
        LEFT JOIN fund_sources fs ON ab.fund_id = fs.id
        LEFT JOIN budget_categories bc ON p.sector = bc.id
        ORDER BY bc.category_name, p.id";
$result = $conn->query($sql);

// Group projects by category name
$projects_by_sector = [];
while ($row = $result->fetch_assoc()) {
    $category_name = $row['category_name'] ?: 'Unspecified';
    if (!isset($projects_by_sector[$category_name])) {
        $projects_by_sector[$category_name] = [];
    }
    $projects_by_sector[$category_name][] = $row;
}
...
<style>
  ...
  #projects-table-container tr.category-header-row td {
    background: linear-gradient(90deg, #3b82f6 0%, #a5b4fc 100%); /* Softer blue gradient */
    color: #1e293b; /* Darker text for better contrast */
    font-size: 1.1rem;
    letter-spacing: 0.02em;
    border-bottom: 2px solid #a5b4fc;
    box-shadow: 0 2px 6px 0 rgba(59,130,246,0.08);
    padding-top: 1.1rem;
