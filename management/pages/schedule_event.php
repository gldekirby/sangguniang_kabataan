...
$events = [];
$sql = "SELECT event_id, event_name, start_date, end_date, start_time, end_time, location, description FROM schedule_events";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $start_date = new DateTime($row['start_date'] . ' ' . $row['start_time']);
        $end_date = new DateTime($row['end_date'] . ' ' . $row['end_time']);
        $current = clone $start_date;
        $event_id = $row['event_id'];
        $event_name = $row['event_name'];
        $location = $row['location'];
        $description = $row['description'];
        // If event is single day, just add it
        if ($start_date->format('Y-m-d') === $end_date->format('Y-m-d')) {
            ...
            // Split into single-day events
            while ($current <= $end_date) {
                $eventStart = clone $current;
                $eventEnd = clone $current;
                // For first day, use original start time, else 00:00:00
                if ($current->format('Y-m-d') === $start_date->format('Y-m-d')) {
                    $eventStartStr = $eventStart->format('Y-m-d\T') . $start_date->format('H:i:s');
                    $eventEndStr = $eventEnd->format('Y-m-d\T') . '23:59:59';
                } elseif ($current->format('Y-m-d') === $end_date->format('Y-m-d')) {
                    $eventStartStr = $eventStart->format('Y-m-d\T') . '00:00:00';
                    $eventEndStr = $eventEnd->format('Y-m-d\T') . $end_date->format('H:i:s');
                } else {
                    $eventStartStr = $eventStart->format('Y-m-d\T') . '00:00:00';
                    $eventEndStr = $eventEnd->format('Y-m-d\T') . '23:59:59';
                }
                $events[] = [
                    'id' => $event_id . '_' . $current->format('Ymd'),
                    'title' => $event_name,
                    'start' => $eventStartStr,
                    'end' => $eventEndStr,
                    'description' => $description,
...
// Helper function to generate a teaser from event name and description
function generateEventTeaser($event_name, $description) {
    // Return the first 100 characters of the description or a default message
    $teaser = trim($description);
    if (empty($teaser)) {
        return "Don't miss out on this exciting event!";
    }
    if (mb_strlen($teaser) > 100) {
        $teaser = mb_substr($teaser, 0, 100) . '...';
    }
    return $teaser;
}
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            ...
            if ($start_date < $today) {
                ...
                $stmt = $conn->prepare("INSERT INTO schedule_events (event_name, start_date, end_date, start_time, end_time, location, description) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssssss", $event_name, $start_date, $end_date, $start_time, $end_time, $location, $description);
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($member = $result->fetch_assoc()) {
                            ...
                            if (preg_match('/^\+639\d{9}$/', $formatted_number)) {
                                ...
                                $start_time_formatted = date('g:i A', strtotime($start_time));
                                $end_time_formatted = date('g:i A', strtotime($end_time));
                                $messageText = "Hey $firstname! Please join our Youth Event \n🌟$event_name is happening!\n\n"
                                    . "📅 When: $start_date to $end_date\n"
                                    . "⏰ Time: $start_time_formatted - $end_time_formatted\n"
                                    . "📍 Where: $location\n\n"
                                    . "What's in it?\n"
                                    .       generateEventTeaser($event_name, $description) . "\n\n"
        ...
        elseif ($action === 'edit' && isset($_POST['event_id'])) {
            ...
            $stmt = $conn->prepare("UPDATE schedule_events SET event_name = ?, start_date = ?, end_date = ?, start_time = ?, end_time = ?, location = ?, description = ? WHERE event_id = ?");
            $stmt->bind_param("sssssssi", $event_name, $start_date, $end_date, $start_time, $end_time, $location, $description, $event_id);
                ...
                if ($result && $result->num_rows > 0) {
                    while ($member = $result->fetch_assoc()) {
                        ...
                        if (preg_match('/^\\+639\\d{9}$/', $formatted_number)) {
                            ...
                            $start_time_formatted = date('g:i A', strtotime($start_time));
                            $end_time_formatted = date('g:i A', strtotime($end_time));
                            $messageText = "Hi $firstname! 📅 Our Your Event \"$event_name\" has been scheduled. It will take place from:\n"
                            . "📅 When: $start_date to $end_date\n"
                            . "⏰ Time: $start_time_formatted - $end_time_formatted\n"
                            . "📍 Where: $location\n\n"
                            . "Event Details:\n$description\n\n"
                            ...
                            try {
                                $client->Send($message);
                                // Removed message insert to messages table (table deleted)
                            } catch (Exception $e) {
                                error_log("SMS send failed to $formatted_number: " . $e->getMessage());
                            }
                        } else {
...
if (isset($_GET['delete'])) {
    ...
    $eventQuery = $conn->prepare("SELECT event_name, start_date, end_date, start_time, end_time, location FROM schedule_events WHERE event_id = ?");
    $eventQuery->bind_param("i", $event_id);
    $eventQuery->execute();
    $eventResult = $eventQuery->get_result();
    $eventDetails = $eventResult->fetch_assoc();
    $eventQuery->close();    if ($eventDetails) {
        $event_name = $eventDetails['event_name'];
        $start_date = $eventDetails['start_date'];
        $end_date = $eventDetails['end_date'];
        $location = $eventDetails['location'];
        // Make sure all are strings, not DateTime objects
        if ($start_date instanceof DateTime) $start_date = $start_date->format('Y-m-d');
        if ($end_date instanceof DateTime) $end_date = $end_date->format('Y-m-d');
    }
    ...
    if ($stmt->execute()) {
        ...
        // ✅ SMS Notification Code for Event Cancellation
        $login = 'KWBUN-';
        $password = '2342Gldekirby@21';
        $client = new Client($login, $password);
        ...
        if ($result && $result->num_rows > 0) {
            while ($member = $result->fetch_assoc()) {
                ...
                if (preg_match('/^\\+639\\d{9}$/', $formatted_number)) {
                    $firstname = $member['first_name'];
                    $messageText = "Hi $firstname, we’re sorry to let you know that our youth event '$event_name' scheduled from $start_date to $end_date at $location has been cancelled. 😔\n\nWe know you were looking forward to it, and we appreciate your understanding. Stay tuned for future updates and events. Thank you for being part of our community!";
                    $message = new Message($messageText, [$formatted_number]);
                    try {
                        $client->Send($message);
                        // Optionally, log or store the cancellation message if needed
                    } catch (Exception $e) {
                        error_log("SMS send failed to $formatted_number: " . $e->getMessage());
                    }
                } else {
...
<head>
    ...
    <style>
        ...
        /* Flex container for calendar and event list */
        .flex-container {
            display: flex;
            flex-direction: column;
            width: 100%;
            padding: 0;
            min-height: 85vh; /* Ensure container is at least as tall as the calendar */
            align-items: stretch;
        }
        ...
        .calendar-section {
            ...
            flex: 1 1 0%;
            min-width: 0;
            min-height: 85vh; /* Ensure calendar section is tall enough */
            display: flex;
            flex-direction: column;
        }
        @media (min-width: 1024px) {
        ...
        /* Add inline style to the event list section to set max-height and enable scrolling */
        .event-list-section {
            max-height: 85vh;
            overflow-y: auto;
        }
   ...
   <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
    ...
    <div class="container-fluid p-0">
        <div class="flex-container">
            ...
            <!-- Event List Section -->
            <div class="event-list-section bg-white border-2 border-indigo-200 rounded-2xl shadow-xl p-6" style="max-height:85vh; overflow-y:auto;">
                <h2 class="text-xl font-bold mb-4 text-indigo-700 flex items-center gap-2">
                    ...
                    <svg class="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <path stroke="currentColor" d="M16 2v4M8 2v4M3 10h18" />
                    </svg>
                    All Scheduled Events
                </h2>
                <div class="space-y-4">
                <?php
                $eventListResult = $conn->query("SELECT event_name, start_date, end_date, start_time, end_time, location, description FROM schedule_events ORDER BY start_date DESC, start_time DESC");
                if ($eventListResult && $eventListResult->num_rows > 0):
                    while ($row = $eventListResult->fetch_assoc()): ?>
                        <div class="bg-indigo-50 border-2 border-indigo-200 rounded-xl shadow-lg p-5 hover:shadow-xl transition">
                            <div class="flex items-center gap-2 text-lg font-semibold text-indigo-800 mb-1">
                                <!-- Event Icon -->
                            ...
                            <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
                                ...
                                <svg class="w-4 h-4 text-indigo-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <circle cx="12" cy="12" r="10" stroke="currentColor" />
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M12 6v6l4 2" />
                                </svg>
                                <?php echo htmlspecialchars($row['start_date']); ?> <?php echo date('g:i A', strtotime($row['start_time'])); ?>
                                &ndash;
                                <?php echo htmlspecialchars($row['end_date']); ?> <?php echo date('g:i A', strtotime($row['end_time'])); ?>
                            </div>
                            <div class="flex items-center gap-2 text-sm text-gray-700 mb-1">
                                ...
                                <span class="font-medium">Location:</span> <?php echo htmlspecialchars($row['location']); ?>
                            </div>
                            <div class="flex items-start gap-2 text-sm text-gray-700">
                                <!-- Description Icon -->
                                <svg class="w-4 h-4 text-indigo-400 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 16h8M8 12h8m-7 8h6a2 2 0 002-2V6a2 2 0 00-2-2H7a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                <span class="font-medium">Description:</span> <?php echo nl2br(htmlspecialchars($row['description'])); ?>
                            </div>
                        </div>
                    <?php endwhile;
                else: ?>
                    <div class="text-center text-gray-500 py-8 bg-white rounded-lg shadow">No events found.</div>
                <?php endif; ?>
            </div>
            </div>
        </div>
            ...
            <form method="POST" action="">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="start_datetime" name="start_datetime" required>
                    </div>
                    <div class="mb-3">
                        <label for="end_datetime" class="block text-sm font-medium text-gray-700">End Date & Time</label>
                        <input type="datetime-local" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="end_datetime" name="end_datetime" required>
                    </div>
                    <div class="mb-3">
        ...
        <div class="modal-container bg-white rounded-lg shadow-lg w-full sm:w-2/3 md:w-1/2 lg:w-1/3">
            ...
            <form method="POST" action="">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="edit_start_datetime" class="block text-sm font-medium text-gray-700">Start Date & Time</label>
                        <input type="datetime-local" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="edit_start_datetime" name="start_datetime" required>
                    </div>
                    <div class="mb-3">
                        <label for="edit_end_datetime" class="block text-sm font-medium text-gray-700">End Date & Time</label>
                        <input type="datetime-local" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" id="edit_end_datetime" name="end_datetime" required>
                    </div>
                    <div class="mb-3">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                views: {
                    dayGridMonth: {
                        dayMaxEventRows: 5, // Increase max events per day
                ...
                eventDisplay: 'block', // Display each event as a block in the calendar day cell
                ...
                dateClick: function(info) {
                    ...
                    document.getElementById('start_datetime').value = info.dateStr + 'T00:00';
                    document.getElementById('end_datetime').value = info.dateStr + 'T00:00';
                    openModal();
                },
                eventClick: function(info) {
                    ...
                    // If endDate is null or same as startDate, show startStr instead of N/A
                    let endStr;
                    if (!endDate || startDate && endDate.getTime() === startDate.getTime()) {
                        endStr = startStr;
                    } else {
                        endStr = endDate.toLocaleDateString('en-US', options);
                    }
        ...
        function openEditModal(event) {
            ...
            if (startDate) {
                document.getElementById('edit_start_datetime').value = startDate.toISOString().slice(0, 16);
            }
            
            if (endDate) {
                document.getElementById('edit_end_datetime').value = endDate.toISOString().slice(0, 16);
            }
