...
if (isset($_POST['add'])) {
    ...
    if ($category_id === '__add_new__' && $new_category_name !== '') {
        // Check if category already exists (case-insensitive)
        $stmt_check = $conn->prepare('SELECT id FROM budget_categories WHERE LOWER(category_name) = LOWER(?)');
        $stmt_check->bind_param('s', $new_category_name);
        $stmt_check->execute();
        ...
        if ($stmt_check->fetch()) {
            ...
            $stmt = $conn->prepare('INSERT INTO budget_categories (category_name) VALUES (?)');
    ...
    if ($category_id) {
        ...
        $fiscal_year = date('Y-m-d H:i:s'); // auto set to current date and time
        ...
        echo '<script>alert("Failed to add budget: Category not set.");</script>';
    }
}
...
if (isset($_POST['update'])) {
    ...
    if ($category_id) {
        ...
        echo '<script>alert("Failed to update budget: Category not set.");</script>';
    }
}
...
if (isset($_POST['ajax_add_category'])) {
    ...
    $stmt = $conn->prepare('SELECT id FROM budget_categories WHERE category_name = ?');
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt->fetch();
        echo json_encode(['success' => true, 'id' => $existing_id]);
        exit;
    }
    $stmt->close();
    // Insert new
    ...
    <section class="flex flex-col flex-grow">
     <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0">
      ...
      <div class="flex flex-wrap items-center gap-4">
       ...
       <select class="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm" id="filter-month">
        <option value="">
         All Months
        </option>
        <option value="01">
     ...
     <div class="space-y-8 min-w-0" id="budgets-container">
      <table class="w-full mx-auto text-sm bg-white">
 <thead>
  <tr>
   <th class="w-12" scope="col">ID</th>
   <th class="min-w-[140px] text-left" scope="col">Program Name</th>
   <th class="min-w-[280px] text-left" scope="col">Description</th>
   <th class="w-36 text-right" scope="col">Allocated Amount</th>
   <th class="w-36 text-center" scope="col">Fiscal Year</th>
   <th class="w-28 text-center" scope="col">Actions</th>
  </tr>
 </thead>
<?php
// Fetch all budgets with category, ordered by category then fiscal year
$sql = "SELECT ab.*, bc.category_name FROM annual_budget ab LEFT JOIN budget_categories bc ON ab.category_id = bc.id ORDER BY COALESCE(bc.category_name, ''), ab.fiscal_year DESC, ab.id DESC";
$all_budgets = $conn->query($sql);

$current_category = null;
$has_no_category = false;
$has_any_budget = false;
$budgets_by_category = [];
foreach ($all_budgets as $row) {
    $cat = $row['category_name'] ?? '';
    if ($cat === null || $cat === '') {
        $cat = 'No Category';
        $has_no_category = true;
    }
    $budgets_by_category[$cat][] = $row;
    $has_any_budget = true;
}
?>
<?php if ($has_any_budget): ?>
    <tbody>
    <?php foreach ($budgets_by_category as $cat_name => $rows): ?>
        <tr class="bg-blue-700 category-header-row">
            <td colspan="6" class="font-bold text-lg text-white py-3">
                <?= htmlspecialchars($cat_name) ?>
            </td>
        </tr>
        <?php foreach ($rows as $row): ?>
        <tr class="hover:bg-blue-50 transition-colors budget-data-row" data-fiscal-year="<?= date('Y', strtotime($row['fiscal_year'])) ?>" data-fiscal-month="<?= date('m', strtotime($row['fiscal_year'])) ?>" data-program-name="<?= htmlspecialchars(strtolower($row['program_name'])) ?>" data-category-name="<?= htmlspecialchars($cat_name) ?>">
            ...
            <td class="text-green-600 font-semibold text-right">
                ₱<?= number_format($row['allocated_amount'], 0) ?>
            </td>
            <td class="text-gray-700 font-mono text-center">
        ...
        <?php endforeach; ?>
    <?php endforeach; ?>
    <tr class="no-budget-row" style="display:none">
        <td class="text-center py-12" colspan="6">
            ...
            <p class="text-gray-500 text-lg font-semibold select-none">No budget found.</p>
        </td>
    </tr>
    </tbody>
<?php else: ?>
...
<?php endif; ?>
  ...
  <form action="" class="fixed top-1/2 left-1/2 max-w-lg w-full bg-white rounded-lg shadow-lg p-6 z-50 -translate-x-1/2 -translate-y-1/2 hidden" id="add-budget-popover" method="post" novalidate="">
   ...
   <div class="mb-4">
    ...
    <select class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm" id="add_category_id" name="category_id" required="">
     ...
     <?php endforeach; ?>
     <option value="__add_new__" class="text-blue-600 font-semibold">+ Add new category...</option>
    </select>
    <input type="text" id="add_new_category_input" name="new_category_name" class="w-full border border-blue-400 rounded-md px-3 py-2 mt-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm hidden" placeholder="Enter new category name..." autocomplete="off" />
   </div>
   <div class="mb-4">
   ...
   <div class="mb-6 hidden">
    ...
    <input class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm" id="add_fiscal_year" name="fiscal_year" type="hidden" value="<?php echo date('Y'); ?>"/>
   </div>
  ...
  <form action="dashboard.php?page=annual_budget" class="fixed top-1/2 left-1/2 max-w-lg w-full bg-white rounded-lg shadow-lg p-6 z-50 -translate-x-1/2 -translate-y-1/2 hidden" id="edit-budget-popover" method="post" novalidate="">
   ...
   <div class="mb-4">
    ...
    <input type="text" id="edit_new_category_input" name="new_category_name" class="w-full border border-blue-400 rounded-md px-3 py-2 mt-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm hidden" placeholder="Enter new category name..." autocomplete="off" />
   </div>
   <div class="mb-4">
   ...
   <div class="mb-6 hidden">
    ...
    <input class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600 text-sm shadow-sm" id="edit_fiscal_year" name="fiscal_year" type="hidden" value="<?php echo date('Y'); ?>"/>
   </div>
   <div class="flex justify-end space-x-3">
   ...
   // Popover open/close logic
    ...
    searchInput.addEventListener('input', () => {
        ...
        categorySections.forEach(section => {
            ...
            let totalRow = null;
            let noBudgetRow = section.querySelector('tr.no-budgets-found-row');
            rows.forEach(row => {
                if (row.classList.contains('bg-gray-100')) {
                    totalRow = row;
                    return;
                }
                if (row.classList.contains('no-budgets-found-row')) {
                    // Hide for now, will show if needed
                    row.style.display = 'none';
                    return;
                }
                const programName = row.getAttribute('data-program-name') || '';
                const matchesSearch = val === '' || programName.toLowerCase().includes(val);
                if (matchesSearch) {
                    row.style.display = '';
                    sectionVisible = true;
                    anyVisible = true;
            ...
            // Show/hide the 'no budgets found' row for this section
            if (!sectionVisible) {
                if (!noBudgetRow) {
                    noBudgetRow = document.createElement('tr');
                    noBudgetRow.className = 'no-budgets-found-row';
                    noBudgetRow.innerHTML = '<td class="px-4 py-6 text-center text-gray-500" colspan="100%">No budgets found.</td>';
                    section.querySelector('tbody').appendChild(noBudgetRow);
                }
                noBudgetRow.style.display = '';
            } else if (noBudgetRow) {
                noBudgetRow.style.display = 'none';
            }
        });
        // Optionally, you can show a global message if no sections have visible rows
    });
    // ...existing code...
    searchInput.addEventListener('input', () => {
        const val = searchInput.value.trim().toLowerCase();
        ...
        filterBudgets(val, document.getElementById('filter-category').value, document.getElementById('filter-year').value, document.getElementById('filter-month').value);
    });
    ...
    clearSearchBtn.addEventListener('click', () => {
        ...
        clearSearchBtn.classList.add('hidden');
        hideSuggestions();
        filterBudgets('', document.getElementById('filter-category').value, document.getElementById('filter-year').value, document.getElementById('filter-month').value);
        searchInput.focus();
    });
    ...
    filterCategory.addEventListener('change', () => {
        filterBudgets(searchInput.value.trim().toLowerCase(), filterCategory.value, filterYear.value, filterMonth.value);
    });
    ...
    // Updated filter/search logic for single table with category headers
function filterBudgets(searchTerm, categoryId, fiscalYear, fiscalMonth) {
    const table = document.querySelector('#budgets-container table');
    if (!table) return;
    ...
    let visibleMonth = false;
    let selectedYear = fiscalYear;
    let selectedMonth = fiscalMonth;
    let yearName = selectedYear;
    let monthName = '';
    if (selectedMonth) {
        const monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        monthName = monthNames[parseInt(selectedMonth, 10) - 1];
    }
    // Hide all rows initially
    rows.forEach(row => row.style.display = 'none');
    // Track which category headers to show
    ...
    rows.forEach((row, idx) => {
        ...
        const rowFiscalMonth = row.getAttribute('data-fiscal-month') || '';
        const rowCategory = row.getAttribute('data-category-name') || '';
        let matchesCategory = true;
        if (categoryId) {
            const catSelect = document.getElementById('filter-category');
            const catOpt = catSelect ? catSelect.querySelector(`option[value='${categoryId}']`) : null;
            const catName = catOpt ? catOpt.textContent.trim() : '';
            matchesCategory = (rowCategory === catName);
        }
        const matchesSearch = searchTerm === '' || programName.includes(searchTerm);
        const matchesYear = fiscalYear === '' || rowFiscalYear === fiscalYear;
        const matchesMonth = fiscalMonth === '' || rowFiscalMonth === fiscalMonth;
        if (matchesSearch && matchesCategory && matchesYear && matchesMonth) {
            row.style.display = '';
            showCategory = true;
            anyVisible = true;
            if (selectedYear && rowFiscalYear === selectedYear) foundForYear = true;
            if (selectedMonth && rowFiscalMonth === selectedMonth) foundForMonth = true;
        }
    });
    // Show category headers if any of their rows are visible
    let lastHeader = null;
    let hasVisibleInCategory = false;
    rows.forEach((row, idx) => {
        if (row.classList.contains('category-header-row')) {
            if (lastHeader && !hasVisibleInCategory) {
            ...
            for (let j = idx + 1; j < rows.length; j++) {
                if (rows[j].classList.contains('category-header-row')) break;
    ...
    // Show 'no budget found' row if nothing is visible
    const noBudgetRow = table.querySelector('.no-budget-row');
    if (noBudgetRow) {
        if (!anyVisible) {
            let msg = 'No budget found.';
            if (selectedYear && !foundForYear) {
                msg = `No budget program found for ${yearName}.`;
            } else if (selectedMonth && !foundForMonth) {
                msg = `No budget program found for ${monthName}.`;
            }
            noBudgetRow.querySelector('p').textContent = msg;
            noBudgetRow.style.display = '';
        } else {
            noBudgetRow.style.display = 'none';
        }
...
// Update event listeners to use new filterBudgets
filterCategory.addEventListener('change', () => {
    filterBudgets(searchInput.value.trim().toLowerCase(), filterCategory.value, filterYear.value, filterMonth.value);
});
filterYear.addEventListener('change', () => {
...
searchInput.addEventListener('input', () => {
    ...
    filterBudgets(val, filterCategory.value, filterYear.value, filterMonth.value);
});
clearSearchBtn.addEventListener('click', () => {
    searchInput.value = '';
    clearSearchBtn.classList.add('hidden');
    hideSuggestions();
    filterBudgets('', filterCategory.value, filterYear.value, filterMonth.value);
    searchInput.focus();
});
// Initial call
filterBudgets('', '', '', '');
    // Category add logic for Add Budget
...
addCategorySelect.addEventListener('change', function() {
    if (this.value === '__add_new__') {
        addNewCategoryInput.classList.remove('hidden');
        addNewCategoryInput.focus();
    } else {
        addNewCategoryInput.classList.add('hidden');
    }
});
addNewCategoryInput.addEventListener('blur', function() {
    if (addNewCategoryInput.value.trim() !== '') {
        fetch('', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'ajax_add_category=1&category_name=' + encodeURIComponent(addNewCategoryInput.value.trim())
        })
        .then(res => res.json())
        .then(data => {
            if (data.success && data.id) {
                ...
                opt.value = data.id;
                opt.textContent = addNewCategoryInput.value.trim();
                addCategorySelect.appendChild(opt);
                addCategorySelect.value = data.id;
                addNewCategoryInput.classList.add('hidden');
...
// Category add logic for Edit Budget
...
editNewCategoryInput.addEventListener('blur', function() {
    if (editNewCategoryInput.value.trim() !== '') {
        fetch('', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'ajax_add_category=1&category_name=' + encodeURIComponent(editNewCategoryInput.value.trim())
        })
        .then(res => res.json())
        .then(data => {
            if (data.success && data.id) {
                const opt = document.createElement('option');
                opt.value = data.id;
                opt.textContent = editNewCategoryInput.value.trim();
                editCategorySelect.appendChild(opt);
                editCategorySelect.value = data.id;
                editNewCategoryInput.classList.add('hidden');
