<html lang="en">
 ...
 <body class="min-h-screen flex flex-col items-center p-4">
  <?php
$conn = new mysqli("localhost", "root", "", "sk_youth");
...
if (isset($_POST['add'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: dashboard.php?page=funds");
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    // Debug logs
    error_log("Update fund called with id: $id, name: $name, description: $description, amount: $amount");
    ...
    if ($stmt->affected_rows === 0) {
        ...
        error_log("Rows updated: " . $stmt->affected_rows);
    }
...
<header class="bg-white shadow p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
    <h1 class="text-3xl font-extrabold text-gray-900 mb-3 sm:mb-0">Funds</h1>
</header>
  <div class="w-full h-full mx-auto bg-white border rounded-lg shadow p-6">
  ...
  <div class="w-full flex flex-col lg:flex-row space-y-6 lg:space-y-0 lg:space-x-6">
   <main class="flex-grow bg-white rounded-lg shadow p-4 overflow-x-auto">
    <!-- Search and Filters -->
    <form action="dashboard.php" class="mb-6 flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-4 sm:space-y-0" method="get" id="auto-search-form">
     <input type="hidden" name="page" value="funds"/>
     <input aria-label="Search fund sources by name or description" class="flex-grow border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" name="search" placeholder="Search by name or description" type="text" value="<?= htmlspecialchars($search) ?>" id="auto-search-input"/>
     <a aria-label="Clear filters and search" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded font-semibold text-gray-700 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-sm" href="dashboard.php?page=funds">
    ...
    <table aria-label="Fund sources table" class="w-full border border-gray-300 rounded-lg text-sm" role="table">
     ...
     <tbody>
      ...
      <tr class="border-b border-gray-200 hover:bg-gray-50">
       ...
       <td class="px-4 py-2 text-right text-green-600 font-semibold">₱<?= number_format($row['amount'], 0) ?></td>
       ...
       <td class="px-4 py-2 text-center space-x-2">
        ...
        <button aria-label="Delete fund source <?= htmlspecialchars($row['name']) ?>" class="delete-btn text-red-600 hover:text-red-800 focus:outline-none hidden" data-id="<?= $row['id'] ?>" type="button">
    ...
    <nav aria-label="Pagination" class="mt-6 flex justify-center space-x-2 flex-wrap">
     <?php
            $queryParams = $_GET;
            $queryParams['page'] = 'funds';
            $queryParams['search'] = $search;
            $queryParams['page_num'] = 1;
            $firstPageUrl = 'dashboard.php?' . http_build_query(array_merge($queryParams, ['page_num' => 1]));
            ?>
     ...
     <?php
            ...
            for ($i = $startPage; $i <= $endPage; $i++):
                ...
                $pageUrl = 'dashboard.php?' . http_build_query($queryParams);
            ?>
     ...
     <?php
            ...
            $lastPageUrl = 'dashboard.php?' . http_build_query($queryParams);
            ?>
  ...
  <!-- Add Fund Source Popover -->
  <div class="popover-backdrop" id="add-popover-backdrop"></div>
  <form action="dashboard.php?page=funds" aria-describedby="add-popover-desc" aria-labelledby="add-popover-title" aria-modal="true" class="popover-form" id="add-fundsource-popover" method="post" novalidate role="dialog">
   <h3 class="text-lg font-semibold mb-4 text-gray-800 flex items-center space-x-2" id="add-popover-title">
    <i class="fas fa-plus"></i>
    <span>Add New Fund Source</span>
   </h3>
   <p class="sr-only" id="add-popover-desc">Form to add a fund source</p>
   <div class="mb-4">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="add-name">Name <span class="text-red-500">*</span></label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="add-name" name="name" required type="text" />
   </div>
   <div class="mb-4">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="add-description">Description</label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="add-description" name="description" type="text" />
   </div>
   <div class="mb-6">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="add-amount">Amount <span class="text-red-500">*</span></label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="add-amount" min="0" name="amount" required step="0.01" type="number" />
   </div>
   <div class="flex justify-end space-x-3">
    ...
    <button class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-sm" id="close-add-popover" type="button">Cancel</button>
   </div>
  </form>

  <!-- Edit Fund Source Popover -->
  <div class="popover-backdrop" id="edit-popover-backdrop"></div>
  <form action="dashboard.php?page=funds" aria-describedby="edit-popover-desc" aria-labelledby="edit-popover-title" aria-modal="true" class="popover-form" id="edit-fundsource-popover" method="post" novalidate role="dialog">
   <h3 class="text-lg font-semibold mb-4 text-gray-800 flex items-center space-x-2" id="edit-popover-title">
    <i class="fas fa-edit"></i>
    <span>Edit Fund Source</span>
   </h3>
   <p class="sr-only" id="edit-popover-desc">Form to edit a fund source</p>
   <input name="id" id="edit-id" type="hidden" />
   <div class="mb-4">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="edit-name">Name <span class="text-red-500">*</span></label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="edit-name" name="name" required type="text" />
   </div>
   <div class="mb-4">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="edit-description">Description</label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="edit-description" name="description" type="text" />
   </div>
   <div class="mb-6">
    <label class="block text-gray-700 font-medium mb-1 text-sm" for="edit-amount">Amount <span class="text-red-500">*</span></label>
    <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" id="edit-amount" min="0" name="amount" required step="0.01" type="number" />
   </div>
   <div class="flex justify-end space-x-3">
    ...
    <button class="px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-sm" id="close-edit-popover" type="button">Cancel</button>
   </div>
  </form>
  ...
  <div aria-describedby="delete-popover-desc" aria-labelledby="delete-popover-title" aria-modal="true" class="popover-form" id="delete-popover" role="dialog">
   ...
   <form action="" class="flex justify-end space-x-3" method="get">
  ...
  <script>
   // Add popover logic
const openAddBtn = document.getElementById('open-popover');
const addPopover = document.getElementById('add-fundsource-popover');
...
if (openAddBtn) openAddBtn.addEventListener('click', function() {
    ...
    const firstInput = addPopover.querySelector('input, select, textarea, button');
    if (firstInput) firstInput.focus();
});
if (closeAddBtn) closeAddBtn.addEventListener('click', function() {
    addPopover.classList.remove('active');
    addBackdrop.classList.remove('active');
    addPopover.reset?.();
});
if (addBackdrop) addBackdrop.addEventListener('click', function() {
    addPopover.classList.remove('active');
    addBackdrop.classList.remove('active');
    addPopover.reset?.();
});
...
const editPopover = document.getElementById('edit-fundsource-popover');
...
editBtns.forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        // Set the correct fund id and values for the edit form only
        const fundId = this.getAttribute('data-id');
        console.log('Editing fund id:', fundId);
        document.getElementById('edit-id').value = fundId;
        document.getElementById('edit-name').value = tr.querySelector('td:nth-child(2)').textContent.trim();
        document.getElementById('edit-description').value = tr.querySelector('td:nth-child(3)').textContent.trim();
        document.getElementById('edit-amount').value = tr.querySelector('td:nth-child(4)').textContent.replace(/[^\d.]/g, '');
        editPopover.classList.add('active');
        editBackdrop.classList.add('active');
        const firstInput = editPopover.querySelector('input, select, textarea, button');
        if (firstInput) firstInput.focus();
    });
});
if (closeEditBtn) closeEditBtn.addEventListener('click', function() {
    ...
    // Clear the edit form after closing
    editPopover.reset?.();
});
if (editBackdrop) editBackdrop.addEventListener('click', function() {
    editPopover.classList.remove('active');
    editBackdrop.classList.remove('active');
    editPopover.reset?.();
});

// Prevent both forms from submitting at once
addPopover.addEventListener('submit', function(e) {
    if (!addPopover.classList.contains('active')) e.preventDefault();
});
editPopover.addEventListener('submit', function(e) {
    if (!editPopover.classList.contains('active')) e.preventDefault();
});
        ...
        element.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Escape') {
                if (!element.classList.contains('hidden') && element.classList.contains('active')) {
                    if (element === addPopover) {
                        addPopover.classList.remove('active');
                        addBackdrop.classList.remove('active');
                    }
                    if (element === editPopover) {
                        editPopover.classList.remove('active');
                        editBackdrop.classList.remove('active');
                    }
    ...
    trapFocus(addPopover);
    trapFocus(editPopover);
    trapFocus(deletePopover);

    // Auto search as you type (client-side, like annual_budget.php)
    const searchInput = document.getElementById('auto-search-input');
    const tableBody = document.querySelector('table[aria-label="Fund sources table"] tbody');
    if (searchInput && tableBody) {
      searchInput.addEventListener('input', function() {
        ...
        rows.forEach(row => {
          // Skip the 'no fund sources found' row
        ...
        // Show/hide the 'no fund sources found' row
        let noRow = tableBody.querySelector('tr.no-funds-found-row');
        if (!anyVisible) {
          if (!noRow) {
            noRow = document.createElement('tr');
            noRow.className = 'no-funds-found-row';
            noRow.innerHTML = '<td class="px-4 py-6 text-center text-gray-500" colspan="6">No fund sources found.</td>';
