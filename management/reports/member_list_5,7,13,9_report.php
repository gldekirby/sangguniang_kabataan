...
function extractSpecificPurok($street) {
    if (preg_match('/(Purok|Prk|Purok|Zone|Sitio)\s*(5|7|13|9)/i', $street, $matches)) {
...
$pdf->Cell(0, 10, 'District 5 - Purok 5, 7, 13, 9', 0, 1, 'C');
$pdf->SetFont('Arial', '', 10);
...
if ($result && $result->num_rows > 0) {
    ...
    if ($hasContent) {
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 8, 'Total Members (Purok 5, 7, 13, 9): ' . $totalApproved, 0, 1, 'L');
        $pdf->Ln(5);
    } else {
        $pdf->Cell(0, 10, 'No members found in Purok 5, 7, 13, 9', 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'No members found in Purok 5, 7, 13, 9', 1, 1, 'C');
}
...
$pdf->Output('I', 'SK_Approved_Members_Purok_5_7_13_9_' . date('Y-m-d') . '.pdf');
?>