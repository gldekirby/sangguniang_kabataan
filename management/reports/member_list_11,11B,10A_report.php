...
function extractSpecificPurok($street) {
    if (preg_match('/(Purok|Prk|Purok|Zone|Sitio)\s*(11|11B|10A)/i', $street, $matches)) {
...
$pdf->Cell(0, 10, 'District 5 - Purok 11, 11B, 10A', 0, 1, 'C');
$pdf->SetFont('Arial', '', 10);
...
if ($result && $result->num_rows > 0) {
    ...
    if ($hasContent) {
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 8, 'Total Members (Purok 11, 11B, 10A): ' . $totalApproved, 0, 1, 'L');
        $pdf->Ln(5);
    } else {
        $pdf->Cell(0, 10, 'No members found in Purok 11, 11B, 10A', 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'No members found in Purok 11, 11B, 10A', 1, 1, 'C');
}
...
$pdf->Output('I', 'SK_Approved_Members_Purok_11_11B_10A_' . date('Y-m-d') . '.pdf');
?>