...
function extractSpecificPurok($street) {
    if (preg_match('/(Purok|Prk|Purok|Zone|Sitio)\s*(3|4|6)/i', $street, $matches)) {
...
$pdf->Cell(0, 10, 'District 2 - PUROK 3, 4, 6', 0, 1, 'C');
$pdf->SetFont('Arial', '', 10);
...
if ($result && $result->num_rows > 0) {
    ...
    if ($hasContent) {
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 8, 'Total Members (Purok 3, 4, 6): ' . $totalApproved, 0, 1, 'L');
        $pdf->Ln(5);
    } else {
        $pdf->Cell(0, 10, 'No members found in Purok 3, 4, or 6', 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'No members found in Purok 3, 4, or 6', 1, 1, 'C');
}
