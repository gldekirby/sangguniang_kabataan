...
function extractSpecificPurok($street) {
    if (preg_match('/(Purok|Prk|Purok|Zone|Sitio)\s*(3A|12|14)/i', $street, $matches)) {
...
$pdf->Cell(0, 10, 'District 3 - Purok 3A, 14, 12', 0, 1, 'C');
$pdf->SetFont('Arial', '', 10);
...
if ($result && $result->num_rows > 0) {
    ...
    if ($hasContent) {
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 8, 'Total Members (Purok 3A, 12, 14): ' . $totalApproved, 0, 1, 'L');
        $pdf->Ln(5);
    } else {
        $pdf->Cell(0, 10, 'No members found in Purok 3A, 12, and 14', 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'No members found in Purok 3A, 12, and 14', 1, 1, 'C');
}
...
$pdf->Output('I', 'SK_Approved_Members_Purok_3_4_6_' . date('Y-m-d') . '.pdf');
?>