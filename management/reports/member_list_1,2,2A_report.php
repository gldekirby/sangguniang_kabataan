...
function extractSpecificPurok($street) {
    if (preg_match('/(Purok|Prk|Purok|Zone|Sitio)\s*(1|2|2A)/i', $street, $matches)) {
...
$pdf->Cell(0, 10, 'District 1 - Purok 1, 2, 2A', 0, 1, 'C');
$pdf->SetFont('Arial', '', 10);
...
if ($result && $result->num_rows > 0) {
    ...
    if ($hasContent) {
        $pdf->SetFont('Arial', 'B', 12);
        $pdf->Cell(0, 8, 'Total Members (Purok 1, 2, 2A): ' . $totalApproved, 0, 1, 'L');
        $pdf->Ln(5);
    } else {
        $pdf->Cell(0, 10, 'No members found in Purok 1, 2, and 2A', 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'No members found in Purok 1, 2, and 2A', 1, 1, 'C');
}
