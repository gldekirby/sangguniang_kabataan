      ...
      <section class="bg-white rounded-lg shadow p-6 overflow-x-auto md:col-span-2">
       ...
       <table class="min-w-full text-sm border border-gray-200 rounded-lg">
        ...
        <tbody>
         ...
         <?php
            // Only sum actual cost for projects with status 'done' for this program
            ...
            $balance_done = $prog['allocated'] - $actual_done;
         ?>
         <tr class="border-b border-gray-200 hover:bg-green-50 transition">
          ...
          <td class="px-5 py-3 text-right text-green-700 font-semibold">
           ₱
           <?= number_format($actual_done, 0) ?>
          </td>
          <td class="px-5 py-3 text-right font-semibold <?php echo ($balance_done < 0) ? 'text-red-700' : 'text-green-700'; ?>">
           ₱
           <?= number_format($balance_done, 0) ?>
          </td>
         </tr>
         <?php endforeach; ?>
         <?php endforeach; ?>
...
let annualBudgetChart, categoryBudgetChart, projectCostChart, addressChart, approvedDayChart, approvedMonthChart, approvedYearChart;
window.addEventListener('DOMContentLoaded', function() {
  ...
  approvedDayChart = Chart.getChart('approvedDayChart');
  approvedMonthChart = Chart.getChart('approvedMonthChart');
  approvedYearChart = Chart.getChart('approvedYearChart');
  pollAnalytics();
});
function pollAnalytics() {
  fetch(window.location.pathname + '?ajax=1')
    ...
    .then(data => {
      ...
      if (approvedDayChart) {
        approvedDayChart.data.labels = Object.keys(data.approvedDayCounts);
        approvedDayChart.data.datasets[0].data = Object.values(data.approvedDayCounts);
        approvedDayChart.update();
      }
      if (approvedMonthChart) {
        approvedMonthChart.data.labels = Object.keys(data.approvedMonthCounts);
        approvedMonthChart.data.datasets[0].data = Object.values(data.approvedMonthCounts);
        approvedMonthChart.update();
      }
      if (approvedYearChart) {
        approvedYearChart.data.labels = Object.keys(data.approvedYearCounts);
        approvedYearChart.data.datasets[0].data = Object.values(data.approvedYearCounts);
        approvedYearChart.update();
      }
    })
