        ...
        if ($action === 'add') {
            ...
            if ($start_date < $today) {
                ...
                if ($stmt->execute()) {
                    ...
                    $result = $conn->query("SELECT contact_number, member_id, firstname FROM members");
...
if (isset($_GET['delete'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<div id='deleteSuccessAlert' class='fixed top-4 right-4 transform translate-x-full transition-transform duration-300 z-50'>
                <div class='bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg shadow-lg flex items-start'>
                    <div class='flex-1'>Event deleted successfully!</div>
                    <button onclick=\"dismissAlert('deleteSuccessAlert')\" class='ml-4 text-green-500 hover:text-green-700'>&times;</button>
                </div>
              </div>
              <script>
                  setTimeout(function() {
                      var element = document.getElementById('deleteSuccessAlert');
                      if (element) element.classList.remove('translate-x-full');
                  }, 100);
                  setTimeout(function() {
                      dismissAlert('deleteSuccessAlert');
                  }, 3000);
...
<head>
    ...
    <link rel="icon" href="bgi/sk_logo.png" type="image/x-icon">
...
<body class="bg-gray-100">
    ...
    <div id="addEventModal" class="hidden fixed inset-0 modal-overlay flex items-center justify-center p-4">
    ...
    <div id="editEventModal" class="hidden fixed inset-0 bg-opacity-50 flex items-center justify-center p-2 sm:p-4">
        <div class="bg-white rounded-lg shadow-lg w-full sm:w-2/3 md:w-1/2 lg:w-1/3">
            <div class="modal-header flex justify-between items-center">
    ...
    <div id="eventDetailsModal" class="hidden fixed inset-0 bg-opacity-50 flex items-center justify-center p-2 sm:p-4">
        <div class="bg-white rounded-lg shadow-lg w-full sm:w-2/3 md:w-1/2 lg:w-1/3">
            <div class="modal-header flex justify-between items-center">
    ...
    <div id="confirmDeleteModal" class="hidden fixed inset-0 bg-opacity-50 flex items-center justify-center p-4 z-50">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    ...
                    if (eventDate < today) {
                        ...
                        info.el.style.backgroundColor = '#f87171'; // Tailwind's red-400
            ...
            // Periodic refresh of calendar events every 30 seconds for realtime update
        ...
        // Add these functions after your existing script tag
        function showSuccessNotification(message) {
            const notificationId = 'notification-' + Date.now();
            const notification = document.createElement('div');
            notification.id = notificationId;
            notification.className = 'fixed top-4 right-4 transform translate-x-full transition-transform duration-300 z-50';
            notification.innerHTML = `
                <div class='bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg shadow-lg flex items-start'>
                    ...
                    <button onclick="dismissNotification('${notificationId}')" class='ml-4 text-green-500 hover:text-green-700'>
        ...
        // Modify the event form submission handlers
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (addEventForm) {
                addEventForm.addEventListener('submit', function(e) {
                    ...
                    fetch('dashboard.php?page=schedule_event', {
                        method: 'POST',
                        body: formData
                    })
                    ...
                    .then(data => {
                        closeModal();
                        showSuccessNotification('Event added successfully!');
                        calendar.refetchEvents();
                        this.reset();
                    })
                    .catch(error => {
            ...
            if (confirmDeleteBtn) {
                confirmDeleteBtn.addEventListener('click', function(e) {
                    ...
                    fetch(`dashboard.php?page=schedule_event&delete=${eventId}`)
                    .then(response => response.text())
                    .then(data => {
