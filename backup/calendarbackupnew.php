    ...
    if ($stmt->execute()) {
        ...
        if ($result && $result->num_rows > 0) {
            while ($member = $result->fetch_assoc()) {
                ...
                if (preg_match('/^\+639\d{9}$/', $formatted_number)) {
                    $firstname = $member['first_name'];
                    $messageText = "Hi $firstname! 📅 The event \"$event_name\" scheduled from $start_date to $end_date at $event_time in $location has been cancelled. We apologize for the inconvenience.";
                    $message = new Message($messageText, [$formatted_number]);
                    try {
                        $client->Send($message);
                    } catch (Exception $e) {
