...
<body class="bg-white w-full">
   ...
   <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
    ...
    <div id="event-list-container" class="mx-auto w-full max-w-7x1 mt-6 mb-8 p-6 bg-gray-50 rounded-lg shadow border border-gray-200">
    ...
    <script>
        ...
        // Populate event list below the calendar
        function renderEventList() {
            ...
            eventList.innerHTML = '';
            if (events.length === 0) {
                eventList.innerHTML = '<li class="py-2 text-gray-500">No events found.</li>';
                return;
            }
            ...
            events.forEach(event => {
                ...
                if (start) {
                    ...
                    const timePart = start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
                    startStr = `${datePart} ${timePart}`;
                }
                if (end) {
                    const datePart = end.toLocaleDateString();
                    const timePart = end.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
                    endStr = `${datePart} ${timePart}`;
                }
                const li = document.createElement('li');
                li.className = 'py-2 flex flex-col sm:flex-row sm:items-center sm:justify-between';
                li.innerHTML = `
                    <div>
                        <span class="font-medium text-indigo-800">${event.title}</span>
                        <span class="block text-xs text-gray-500">${startStr}${endStr ? ' - ' + endStr : ''}</span>
                        <span class="block text-xs text-gray-500">${event.extendedProps.location ? '📍 ' + event.extendedProps.location : ''}</span>
                    </div>
                    <div class="mt-1 sm:mt-0 flex gap-2">
                        <button class="text-xs text-blue-600 hover:underline" onclick="calendar.getEventById('${event.id}').setProp('selected', true); openEventDetailsModalFromList('${event.id}')">Details</button>
                    </div>
                `;
        ...
        // Helper to open event details from the list
        function openEventDetailsModalFromList(eventId) {
        ...
        // Call renderEventList after calendar render and after refetch
        document.addEventListener('DOMContentLoaded', function() {
            // ...existing code...
            calendar.render();
            renderEventList();
            calendar.on('eventsSet', renderEventList);
            // ...existing code...
        });
        // ...existing code...
    </script>
