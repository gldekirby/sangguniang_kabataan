...
<main class="  flex-grow w-full border h-full mx-auto px-4 py-6 space-y-10" id="projects-container">
  ...
  <div class="space-y-8 min-w-0" id="projects-table-container">
    <table class="w-full mx-auto text-sm">
      ...
      <?php } endforeach; ?>
      <?php if (!$has_any_project): ?>
        <tbody>
          <tr class="no-projects-found-row">
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    function filterProjects() {
      ...
      // Show/hide the no projects row
      if (noProjectsRow) {
        // Remove the row from its current parent (if any)
        if (noProjectsRow.parentNode) {
          noProjectsRow.parentNode.removeChild(noProjectsRow);
        }
        ...
        if (visibleProjectRows.length === 0) {
          // Remove any existing no-result tbody
          const tbodies = Array.from(table.querySelectorAll('tbody'));
          tbodies.forEach(tb => {
            if (tb.querySelector('.no-projects-found-row')) tb.remove();
          });
          // Create a new tbody and append the no result row
          const newTbody = document.createElement('tbody');
          newTbody.appendChild(noProjectsRow);
